...
import random
import numpy as np
from model import Dueling_DQN as DQN, DQNLoss
	...
	for i in range(iter_num):
		...
		for j in range(step):
			...
			if len(replay) > batch_size:
				...
				# 计算目标r+gamma * Q
				action_q = dqn(train_newstate)
				aciton_q_target = dqn_target(train_newstate)
				action = torch.max(action_q, 1)[1]
				index_x = torch.Tensor(np.arange(0, action.shape[0], 1)).long()
				index = (index_x, action)
				now_q = reward + gamma * aciton_q_target[index]
				# 终点的情况下不乘gamma
				...
				pit += 1
				if pit % 10 == 0:
					torch.save(dqn.state_dict(), './DuelDQN_checkpoint/DuelDQN.pth')
					dqn_target.load_state_dict(torch.load('./DuelDQN_checkpoint/DuelDQN.pth'))
				if pit % 10000 == 0:
